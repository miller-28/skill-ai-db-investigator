...
function readSql(argv) {
  ...
  if (fi !== -1 && argv[fi + 1]) return readFileSync(resolveSqlFilePath(argv[fi + 1]), 'utf8').trim();
  const rest = argv.filter((argument) => argument !== '--file');
  if (rest.length === 1 && rest[0].startsWith('@')) return readFileSync(resolveSqlFilePath(rest[0]), 'utf8').trim();
  if (rest.length > 0) return rest.join(' ').trim();
  if (!process.stdin.isTTY) return readFileSync(0, 'utf8').trim();
  throw new Error('No SQL: pass query text, --file, @file, or stdin.');
}
...
ensureUserContextStore({ bridgeRoot, envName: resolvedEnvName, engine: 'mariadb' });
...
let connection;
try {
  connection = await mariadb.createConnection(connectionString);
