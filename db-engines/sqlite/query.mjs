...
const { value: sqliteFilePath, source, envName: resolvedEnvName } = resolveEngineConnection({
  bridgeRoot,
  engine: 'sqlite',
  envNameFlag: envName,
});

ensureUserContextStore({ bridgeRoot, envName: resolvedEnvName, engine: 'sqlite' });
...
let database;
try {
  database = new Database(sqliteFilePath, { readonly: true, fileMustExist: true });
  const dbVersion = database.prepare('SELECT sqlite_version() AS version').get()?.version || 'unknown';
